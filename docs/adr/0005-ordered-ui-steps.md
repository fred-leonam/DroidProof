...
The sample uses a local button listener to change `DroidProof ready` to `DroidProof action completed`. Activity creation and a new launch intent establish the initial sample state. Both v1 samples remain; v2 includes passing and intentionally wrong-text scenarios. The JVM suite uses synthetic hierarchies and fake device operations and requires no emulator.
